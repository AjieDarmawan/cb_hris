...
$page = "rekap_simulasi_new_reward_v4.php";
...
if(!empty($_SESSION['fday']) &&
   ...
   !empty($_SESSION['fdivisi'])){
    ...
    if($_SESSION['fdivisi'] == "unit"){
		...
		AND a.kar_nik IN ('SG.0609.2021','SG.0318.2016')
        ...
        while($data = mysql_fetch_assoc($query)){
                ...
                foreach($data_parse as $key => $val){
					if($val['status'] != 'KP'){
						$data_source[$key] = array('biobid' => $val['biobid'],
							  ...
							  'status_her' => $val['status_her'], //reward heregis di buka lagi
							  'biaya_formulir' => $val['biaya_formulir'],
			...
			if(!empty($data_source)){			
                foreach($data_source as $key => $val){
					if(($val['kodept'] == strtoupper($grdkpt)) && ($val['status_reg'] !='NOT-PROMO' || $val['status_reg'] !='GSF-PROMO')){
                ...
                foreach($data_parse as $key => $val){
					if($val['status'] != 'KP'){
						$data_source[$key] = array('biobid' => $val['biobid'],
							  ...
							  'status_her' => $val['status_her'], //reward heregis dibuka lg
							  'biaya_formulir' => $val['biaya_formulir'],
							  'formulir' => $val['formulir'],
							  'logika' => $val['logika']);
					}
