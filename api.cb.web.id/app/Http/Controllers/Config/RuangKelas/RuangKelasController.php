...
use App\Http\Controllers\Config\RuangKelas\RuangKelasModel as MainModel;
...
class RuangKelasController extends Controller 
{
	...
	*/
	public function listdata(Request $request) 
	{
		...
		$result = MainModel::select('kode_ruang', 'nama_ruang')->paginate($limit)->toArray();
		...
		if($request->input('type') !== null && $request->input('type') == 'slim') 
		{
			foreach($result['data'] as $k => $v) 
			{
				...
				if($request->input('type') !== null && $request->input('type') == 'slim') 
				{	
					$resdata['id'] = $v['kode_ruang'];
					$resdata['name']= $v['nama_ruang'];
				}
		...
		if($request->input('type') !== null && $request->input('type') == 'full') 
		{
			...
			foreach($result['data'] as $k => $v) 
			{
				$resdata = array();
				$resdata['no'] = $no;
				$resdata['kode'] = $v['kode_ruang'];
				$resdata['nama']= $v['nama_ruang'];
