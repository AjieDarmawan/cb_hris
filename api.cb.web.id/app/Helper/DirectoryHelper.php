...
if(!function_exists('apps_perm'))
{
        ...
        * @return string
        */
        function apps_perm($origin=null)
        {
			$path = app()->basePath('apps');
			$perm = fopen($path, "r") or die("Unable to open file!");
			$rest = @json_decode(fread($perm,filesize($path)), true);
			fclose($perm);
			
			if(isset($rest[$origin]['active']) && ($rest[$origin]['active'])) {
				return $rest[$origin];
			} 
