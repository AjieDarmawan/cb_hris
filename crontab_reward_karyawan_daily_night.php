...
function outputProgress($current, $total) {
    echo "<strong style='position: absolute;z-index:$current;background:#FFF;'>Proses: " . round($current / $total * 100) . "% </strong>";
    myFlush();
    slaap(0.5);
}
...
// $r_awal_ori = "2020-05-07";
// $r_sekarang_ori = "2020-05-07";

$r_awal_ori = $kemarin;
$r_sekarang_ori = $hariini;

$current = 0;
$karyawanArr = array();
$logika_value = "1"; //logika

$kar_tampil_div_in = $kar->kar_tampil_div_in_logika($logika_value);
...
while($kar_data_div_in = mysql_fetch_assoc($kar_tampil_div_in)){
    ...
    $tb_rwd = "rwd_data_karyawan";
    $nik = $kar_data_div_in['kar_nik'];
    $nama = $kar_data_div_in['kar_nm'];
    
    if($kar_data_div_in['kar_logika']==1){
        $identify = "_KAR";
    }else{
        $identify = "";
    }
    
    $divisi = $kar_data_div_in['div_id'].$identify;
    $bdc = $BDC_datares;
