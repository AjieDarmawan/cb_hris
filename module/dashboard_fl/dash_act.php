...
if(isset($_POST['babsmasuk'])){
				...
				*/
				...
				if ($shift == "Shift Pagi"){
					...
					$abs_shift="Shift Pagi";
						if($waktu_jam_menit > $jam_telat_pagi){
							$abs_rwd_masuk="Telat";
                                                        $abs_point=30;
							$abs_masuk=$abf->abs_masuk_telat($abs_masuk,$abs_ip,$abs_tgl_masuk,$abs_alasan_masuk,$abs_shift,$abs_rwd_masuk,$abs_point,$kar_id);
						...
						elseif($waktu_jam_menit < $jam_tepat_pagi){
							$abs_rwd_masuk="Rajin";
                                                        $abs_point=80;
							$abs_masuk=$abf->abs_masuk($abs_masuk,$abs_ip,$abs_tgl_masuk,$abs_shift,$abs_rwd_masuk,$abs_point,$kar_id);
							$chc_insert_masuk=$chc->chc_insert_masuk($abs_tgl_masuk, $chc_nik, $chc_nm, $time, $latlong, $status_radius, $location, $jarak, $kar_id);
                                                        //Notify
							$ntf_id=mysql_insert_id();
                                                        $ntf_act="Absen Masuk";
				...
				if($abs_masuk){
					...
					$abs_dtl_tgl=$date;
					$abs_dtl_sts="H";
					$abs_dtl_tampil=$abf->abs_dtl_tampil($kar_id,$abs_dtl_tgl);
				  	$abs_dtl_cek=mysql_num_rows($abs_dtl_tampil);
					if($abs_dtl_cek > 0){
						$abs_dtl_update=$abf->abs_dtl_update($abs_dtl_tgl,$abs_dtl_sts,$kar_id);
					}else{
						//$abs_dtl_insert=$abf->abs_dtl_insert($abs_dtl_tgl,$abs_dtl_sts,$kar_id);
						$abs_dtl_insert=$abf->abs_dtl_insert_full($abs_dtl_tgl,$abs_dtl_sts,$abs_dtl_type,$location,$kar_id);
					}
...
elseif(isset($_POST['babspulang'])){
	...
	$abs_tampil_kar=$abf->abs_tampil_kar($kar_id,$abs_tgl_masuk);
	...
	//if($abs_data['abs_ip']==$ip_data['ip_nm']){
		...
		$jamtepat = substr($jam_cepat_pagi , 0,2).":30";
			...
			$abs_pulang=$abf->abs_pulang_cepat($abs_pulang,$abs_ip,$abs_tgl_pulang,$abs_alasan_pulang,$abs_rwd_pulang,$abs_point,$kar_id,$abs_tgl_masuk);
			...
			$abs_pulang=$abf->abs_pulang($abs_pulang,$abs_ip,$abs_tgl_pulang,$abs_rwd_pulang,$abs_point,$kar_id,$abs_tgl_masuk);
...
elseif(isset($_POST['babspulangmalam'])){
	$abs_tgl_las=$kemarin;
	$abs_tampil_las=$abf->abs_tampil_las($kar_id,$abs_tgl_las);
	...
	//if($abs_data['abs_ip']==$ip_data['ip_nm']){
	    ...
	    if($abs_data['abs_shift']=="Shift Pagi"){
	    	if($waktu_jam_menit < $jam_cepat_pagi){
	    		$abs_rwd_pulang="Izin";
	    		$abs_point=($abs_data['abs_point']+30);
			$abs_pulang=$abf->abs_pulang_cepat($abs_pulang,$abs_ip,$abs_tgl_pulang,$abs_alasan_pulang,$abs_rwd_pulang,$abs_point,$kar_id,$abs_tgl_las);
	    		...
	    		$abs_rwd_pulang="Loyal";
	    		$abs_point=($abs_data['abs_point']+80);
			$abs_pulang=$abf->abs_pulang($abs_pulang,$abs_ip,$abs_tgl_pulang,$abs_rwd_pulang,$abs_point,$kar_id,$abs_tgl_las);
