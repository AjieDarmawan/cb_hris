<?php require('module/kasbon_approve_coba/ksbn_conf.php'); ?>
		...
		<div class="col-xs-12">
			...
			<div class="box">
				...
				<div class="box-body">
					...
					<table id="kasbonApproveTblCoba" class="table table-striped table-bordered" style="width:100%">
  ...
  <div class="modal-dialog modal-lg">
  			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel"><?php echo ' Detail ';?> </h4>
				</div>
...
<div  id="myRekapDetail2" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" 
     aria-labelledby="myLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
  			<div class="modal-content">
				<div class="modal-header">
				...
				<!-- memulai untuk konten dinamis -->
				<!-- lihat id="data_siswa", ini yang di pangging pada ajax di bawah -->
				<div class="modal-body  table-responsive" id="data_barang2"></div>
				<!-- selesai konten dinamis -->
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
				</div>
...
$(document).ready(function(){
	...
	// APPROVAL KASBON
if(typeof document.getElementById("kasbonApproveTblCoba") !== 'undefined' && document.getElementById("kasbonApproveTblCoba") !== null) {
		var cur_url = window.location.href;
		var table = $('#kasbonApproveTblCoba').DataTable({ 
			processing: true,
			serverSide: true,
			order: [],
			ajax:{
				"url": 'module/kasbon_approve_coba/ksbn_act.php',
				"type":"POST",
				"data":function(data){
					data.mode='list';
			...
			columnDefs: [
				{"targets": 0, "className": "text-left", "searchable": false, "orderable": false},
	...
	$('.search-data-table').keyup( function() {
		$("#kasbonApproveTbl_filter :input").val($(this).val()).keyup();
	});
	...
	$('#approval_kasbon').submit(function(e) {
		$.ajax({
			type: 'POST',
			url: 'module/kasbon_approve_coba/ksbn_act.php',
			...
			success: function(data) {
				...
				if(json.status == '1') {
					table.ajax.reload();
					$('#modal-approve-pengajuan-kasbon').modal('hide');
					$('#approval_kasbon').trigger("reset");
				} else {
					alert(json.msg);
				}
...
function CekPODetail(data){
			 ...
			 var kampus = res[2];
			 $.ajax({
				url: './module/kasbon_approve_coba/view_detail_acc.php',	
				method: 'post',		
...
function CekPODetail2(data){
			 ...
			 var kampus = res[2];
			 $.ajax({
				url: './module/kasbon_pengajuan_coba/view_detail.php',	
				method: 'post',		
				data: {nik:nik, tgl:tgl,kampus:kampus },		
				success:function(data){		
					// alert(data);
					$('#data_barang2').html(data);	
					$('#myRekapDetail2').modal("show");	
				},
			    error: function(){
					$('#myRekapDetail2').modal("hide");	
					//error_data();			
			  	}		
