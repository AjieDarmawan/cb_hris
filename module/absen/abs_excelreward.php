...
$objPHPExcel->getActiveSheet()->setTitle('Reward Absen');
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if($jumdat>0) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'REWARD ABSEN');
	...
	$objPHPExcel->getActiveSheet()->mergeCells('D6:D7');
        ...
        for($i=0,$j='E',$h=1,$k='D',$s='C';$i<$daysInMonth;$i++,$j++,$h++,$k++,$s++) {
            ...
            //$objPHPExcel->getActiveSheet()->setCellValue($j.'8', 'MASUK');
            //$objPHPExcel->getActiveSheet()->setCellValue($j.'8', 'REWARD');
            //$objPHPExcel->getActiveSheet()->setCellValue($j.'8', 'PULANG');
            //$objPHPExcel->getActiveSheet()->setCellValue($j.'8', 'REWARD');
	...
	*/
	...
	for ($i = 1; $i <= $jumdat; $i++) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$brs, $arr["div_nm"][$i]);
                ...
                for($i1=1,$j1='E';$i1<=$daysInMonth;$i1++,$j1++) {
						...
						$abs_masuk=$detrewardabsen[$kar_id__][$abs_tgl_masuk]["abs_masuk"];
						$abs_pulang=$detrewardabsen[$kar_id__][$abs_tgl_masuk]["abs_pulang"];
						$abs_rwd_masuk=$detrewardabsen[$kar_id__][$abs_tgl_masuk]["abs_rwd_masuk"];
						$abs_rwd_pulang=$detrewardabsen[$kar_id__][$abs_tgl_masuk]["abs_rwd_pulang"];
                                                
                                                if($abs_tgl_masuk==$detrewardabsen[$kar_id__][$abs_tgl_masuk]["abs_tgl_masuk"]){

							$objPHPExcel->getActiveSheet()->setCellValue($j1.$brs, 'M : '.$abs_masuk.' ('.$abs_rwd_masuk.') | P : '.$abs_pulang.' ('.$abs_rwd_pulang.')');
	...
	$objPHPExcel->getActiveSheet()->getStyle('D'.$brsAWAL.':'.'D'.$brsAKHIR)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	...
	$objPHPExcel->getActiveSheet()->getStyle('A'.$brsAKHIR.':'.$k.$brsAKHIR)->getFont()->setSize(9);
        $objPHPExcel->getActiveSheet()->getStyle('E'.$brsAWAL.':'.$k.$brsAKHIR)->getFont()->setSize(9);
