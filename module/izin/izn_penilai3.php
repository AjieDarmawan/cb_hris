...
if($kpi_cek_id > 0 && $kar_id==$kpi_data_id['kpi_penilai3']){
  
if($kpi_data_id['kpi_sts']=='Z'){
  $p3_readonly = "";
  $p3_disabled = "";
}else{
  ...
  <section class="invoice col-md-7">
	...
	<address>
	  <div class="form-group">
	    ...
	    <div class="col-md-12">
	      <input type="text" name="kpi_tanggal" class="form-control" id="kpi_tanggal" value="<?php echo $kpi_data_id['kpi_tanggal'];?>" placeholder="Tanggal Penilaian" data-inputmask="'alias': 'yyyy-mm-dd'" data-mask style="width:182;" required <?php echo $p3_tanggal;?>>
	    </div>
	  </div>
		...
		<tbody>
		    ...
		    for($i=0;$i<count($exp1);$i++){
			  ...
			  if($bdc_cek !== false) {
			    ...
			    $kps_aktual1 = '<input type="text" class="form-control" name="kpi_data1['.$no.']" value="'.$exp2[4].'" style="width:60px;" data-max="'.$kpi_sasaran_data['kps_target'].'" onkeyup="cekMax(this)" '.$p1_readonly.'>';
			    ...
			    $kps_aktual1 = '<select class="form-control" style="width:60px;" '.$p1_disabled.'>';
			    ...
			    $kps_aktual1 .= '</select>';
			    $kps_aktual1 .= '<input type="hidden" class="form-control" name="kpi_data1['.$no.']" value="'.$exp2[4].'" style="width:60px;" readonly>';

			    $kps_aktual2 = '<select class="form-control" name="kpi_data2['.$no.']" style="width:60px;" '.$p2_disabled.'>';
			    $kps_aktual2 .= '<option value="" selected></option>';
			    $kpi_point_jenis = $kpi->kpi_point_jenis($kpb_jenis);
			    while($kpi_point_data=mysql_fetch_array($kpi_point_jenis)){
			      if($kpi_point_data['kpb_nilai'] == $exp2[5]){
				$selected = "selected";
			      }else{
				$selected = "";
			      }
			      $kps_aktual2 .= '<option value="'.$kpi_point_data['kpb_nilai'].'" '.$selected.'>'.$kpi_point_data['kpb_kd'].'</option>';
			    }
			    $kps_aktual2 .= '</select>';
	...
	<div class="table-responsive">
		...
		?>
		    <input type="radio" name="kpi_ditetapkan" value="<?php echo $value;?>" class="flat-red kpi_ditetapkan" checked/> <strong class="text-blue"><?php echo $caption;?></strong><br>
		<?php }else{?>
		    <input type="radio" name="kpi_ditetapkan" value="<?php echo $value;?>" class="flat-red kpi_ditetapkan" <?php echo $p3_ditetapkan;?>/> <?php echo $caption;?><br>
		<?php }}?> 
	...
	<div class="table-responsive">
	  ...
	  ?>
	  <textarea name="kpi_saranperbaikan" class="form-control" <?php echo $p3_ditetapkan;?>><?php echo $kpi_data_id['kpi_saranperbaikan'];?></textarea>
	  <?php }else{ ?>
	  <textarea name="kpi_saranperbaikan" id="kpi_saranperbaikan" class="form-control" <?php echo $p3_ditetapkan;?> style="display: none" disabled><?php echo $kpi_data_id['kpi_saranperbaikan'];?></textarea>
	  <?php }?>
	</div>
	...
	<br>
	  ...
	  if($kpi_data_id['kpi_reward'] >= 40){
	    $kpi_reward = "(".$kpi_data_id['kpi_reward'].") GET REWARD";
	    echo "<strong class='text-green'>".$kpi_reward."</strong>";
	  }else{
	    $kpi_reward = "(".$kpi_data_id['kpi_reward'].") NO REWARD";
	    echo "<strong class='text-red'>".$kpi_reward."</strong>";
	  }
	  ?>
	  <input type="hidden" name="kpi_sts_reward" value="<?php echo $kpi_reward;?>">
	<br><br>
	...
	<strong>Skor:</strong>
	  ...
	  ?>
	  <input type="hidden" name="kpi_skor" value="<?php echo round($totskor, 2);?>">
	<br>
      </div><!-- /.col -->
	...
	<?php }else{?>
	<button name="bapproved" id="bapproved" class="btn btn-success pull-right" <?php echo $p3_disabled;?>><i class="fa fa-thumbs-up"></i> Approved</button>
	<?php }?>
      </div>
    </div>
