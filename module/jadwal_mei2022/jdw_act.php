...
if(isset($_POST['bimport'])){
	...
	$upload_dir = "file_jadwal/";
        ...
        $file_name = "JADWAL_052022".$extension;
	...
	if (!move_uploaded_file($_FILES['jdw_file']['tmp_name'], $file_path)) {
	    ...
	    //exit();
			...
			$_kar_tampil = $kar->kar_tampil_username($jdw_nik);
			$_kar_data = mysql_fetch_assoc($_kar_tampil);
			$jdw_divisi = $_kar_data['div_nm'];
                        

                        $jdw_insert = $jdw->jdw_insert($jdw_blnthn,$jdw_username,$jdw_nik,$jdw_nama,$jdw_divisi,$jdw_zona,$jdw_wilayah,$jdw_data);
...
$jdw_aktif_blnthn = $jdw->jdw_aktif_mei2022();
...
while($jdw_data = mysql_fetch_assoc($jdw_tampil)){
    ...
    $dataArr[]=array('jdw_nik'=>$jdw_data['jdw_nik'],
		     ...
		     'jdw_divisi'=>$jdw_data['jdw_divisi'],
