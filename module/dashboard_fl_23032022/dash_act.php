...
if(isset($_POST['babsmasuk'])){
				...
				if ($shift == "Shift Pagi"){
					...
					$abs_shift="Shift Pagi";
						if($waktu_jam_menit > $jam_telat_pagi){
							$abs_rwd_masuk="Telat";
                                                        $abs_point=30;
							$abs_masuk=$abf->abs_masuk_telat($abs_masuk,$abs_ip,$abs_tgl_masuk,$abs_alasan_masuk,$abs_shift,$abs_rwd_masuk,$abs_point,$kar_idfl);
                                                        ...
                                                        $ntf_tgl=$date;
                                                        $ntf_jam=$time;
                                                        //$ntf_insert=$ntf->ntf_insert($ntf_act,$ntf_isi,$ntf_ip,$ntf_tgl,$ntf_jam,$kar_idfl);
                                                        //End Notify
						...
						elseif($waktu_jam_menit < $jam_telat_pagi){
							$abs_rwd_masuk="Rajin";
                                                        $abs_point=80;
							$abs_masuk=$abf->abs_masuk($abs_masuk,$abs_ip,$abs_tgl_masuk,$abs_shift,$abs_rwd_masuk,$abs_point,$kar_idfl);
				...
				if($abs_masuk){
					if(empty($acc_datafl['acc_sts'])){
						$acc_id=$acc_datafl['acc_id'];
						$acc_sts="A";
						$acc_update_sts=$afl->acc_update_sts($acc_id,$acc_sts);
					}

					$abs_dtl_tgl=$date;
					$abs_dtl_sts="H";
					$abs_dtl_tampil=$abf->abs_dtl_tampil($kar_idfl,$abs_dtl_tgl);
				  	$abs_dtl_cek=mysql_num_rows($abs_dtl_tampil);
					if($abs_dtl_cek > 0){
						$abs_dtl_update=$abf->abs_dtl_update($abs_dtl_tgl,$abs_dtl_sts,$kar_idfl);
					}else{
						$abs_dtl_insert=$abf->abs_dtl_insert($abs_dtl_tgl,$abs_dtl_sts,$kar_idfl);
					}
					...
					$kar_update_location=$kar->kar_update_location($kar_idfl,$ktr_id_,$unt_id_);
					//$kar_update_location=$kar->kar_update_location($kar_idfl,$location);
					
					
				        $kduser=$acc_datafl['acc_username'];
				        $nmfoto = date('YmdHis');
				        ...
				        if(!empty($_POST['mydata'])){
				           $acc_img_update=$afl->acc_img_update($acc_img,$kar_idfl);
				           if(!empty($acc_datafl['acc_img'])){
				             $dir_img="module/profile/img/$acc_datafl[acc_img]";	
				             unlink($dir_img);
				           }
...
elseif(isset($_POST['babspulang'])){
	...
	//if($abs_data['abs_ip']==$ip_data['ip_nm']){
		...
		$jamtepat = substr($jam_cepat_pagi , 0,2).":30";
			...
			$abs_pulang=$abf->abs_pulang_cepat($abs_pulang,$abs_ip,$abs_tgl_pulang,$abs_alasan_pulang,$abs_rwd_pulang,$abs_point,$kar_idfl,$abs_tgl_masuk);
			...
			$abs_pulang=$abf->abs_pulang($abs_pulang,$abs_ip,$abs_tgl_pulang,$abs_rwd_pulang,$abs_point,$kar_idfl,$abs_tgl_masuk);
	...
	//if($abs_data['abs_ip']==$ip_data['ip_nm']){
	    ...
	    if($abs_data['abs_shift']=="Shift Pagi"){
	    	if($waktu_jam_menit < $jam_cepat_pagi){
	    		$abs_rwd_pulang="Izin";
	    		$abs_point=($abs_data['abs_point']+30);
			$abs_pulang=$abf->abs_pulang_cepat($abs_pulang,$abs_ip,$abs_tgl_pulang,$abs_alasan_pulang,$abs_rwd_pulang,$abs_point,$kar_idfl,$abs_tgl_las);
	    		...
	    		$abs_rwd_pulang="Loyal";
	    		$abs_point=($abs_data['abs_point']+80);
			$abs_pulang=$abf->abs_pulang($abs_pulang,$abs_ip,$abs_tgl_pulang,$abs_rwd_pulang,$abs_point,$kar_idfl,$abs_tgl_las);
...
elseif(isset($_POST['bsave'])){
	if(!empty($pos_atc)){
			...
			if(count($errors) === 0) {
			    $pos_insert_atc=$pos->pos_insert_atc($pos_msg,$pos_atc,$pos_tgl,$pos_jam,$mrk_id,$kar_idfl);
            ...
            $pos_insert=$pos->pos_insert($pos_msg,$pos_tgl,$pos_jam,$mrk_id,$kar_idfl);
