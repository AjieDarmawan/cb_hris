...
if(isset($_POST['babsmasuk'])){
				...
				if($kar_id){
					if(($exp_data[$date_int] !== "WFH" && $exp_data[$date_int] !== "WFH-M" && $abs_dtl_type == "WFH") || ($exp_data[$date_int] == "WFH" && $abs_dtl_type == "WFO")){
						echo "<script>alert('Pilih Lokasi sesuai jadwal, Terima kasih');document.location='x_menu.php';</script>'";
						exit;
					}
				}
				*/
				...
				if($_SESSION['kar'] == 247 || $_SESSION['kar'] == 413){
					if($status_radius == "DI LUAR RADIUS"){
						echo "<script>alert('Sistem telah menditeksi Anda diharuskan absensi di Kampus Terima kasih');document.location='x_menu.php';</script>'";
						exit;
					}
				}
				...
				if($abs_masuk){
					...
					echo"<script>document.location='x_menu.php';</script>";
				}else{
					echo"<script>alert('Absen Masuk Failed');document.location='x_menu.php';</script>";
				}
...
elseif(isset($_POST['babspulang'])){
	...
	//if($abs_data['abs_ip']==$ip_data['ip_nm']){
		...
		if($abs_pulang){
			...
			echo"<script>alert('Absen Pulang Failed');document.location='x_menu.php';</script>";
		}
	//}else{
		//echo"<script>alert('Maaf Anda tidak dapat melakukan Absen Pulang sebelum mengkonfirmasikan Absen Masuk Anda ke Departement SDM');document.location='x_menu.php';</script>";
	//}
}
	...
	if(!empty($pos_atc)){
			...
			if(count($errors) === 0) {
			    ...
			    foreach($errors as $error) {
			        echo "<script>alert('$error');document.location='x_menu.php';</script>";
			    }
			    die(); 
			}
...
elseif(isset($_POST['checkpoint2post'])){
	...
	echo "<script>alert('Terima kasih, selamat bekerja kembali');document.location='x_menu.php';</script>";
}
elseif(isset($_POST['checkpoint3post'])){
	...
	echo "<script>alert('Terima kasih, semangat ya kerjanya');document.location='x_menu.php';</script>";
}
?>