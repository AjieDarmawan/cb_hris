...
<?php if($fpk_cek_id > 0 && $kar_id==$fpk_data_id['fpk_mengetahui']){?>
		 ...
		 <?php
	      ...
	      <br>
			...
			if($fpk_data_id['fpk_sts']=="Z"){
		      ...
		      <input type="radio" name="fpk_ditetapkan" value="<?php echo $value;?>" class="flat-red" id="fpk_ditetapkan" checked <?php echo $konfirm;?>/> <?php echo $caption;?><br>
		      <?php }else{?>
		      <input type="radio" name="fpk_ditetapkan" value="<?php echo $value;?>" class="flat-red" id="fpk_ditetapkan" <?php echo $konfirm;?>/> <?php echo $caption;?><br>
		      <?php }}}?>
              </div>
            </div><!-- /.col -->
          </div><!-- /.row -->
	...
	<hr>
	  ...
	  <!-- info row -->
	    ...
	    <div class="col-sm-7 invoice-col">
		  ...
		  $idkonfirm= $pecah2[0];		  
		  $stskonfirm= $pecah2[1];		  
		  $arr["idkonfirm"]= $idkonfirm;
		  $arr["stskonfirm"]= $stskonfirm;	
		  
		  if($arr["stskonfirm"]=="Y"){
		     $lblkonfirm="fa-check-square-o";
		  }else{
		     $lblkonfirm="fa-square-o";
		  }
		  
		  $kar_kfm=$arr["idkonfirm"];
		  $kar_tampil_kfm=$kar->kar_tampil_id($kar_kfm);
		  $kar_data_kfm=mysql_fetch_array($kar_tampil_kfm);

	      ?>	  
	      <i class="fa <?php echo $lblkonfirm;?>"></i> <?php echo $kar_data_kfm['kar_nm'];?><br>
	      <?php }?>
	    </div>
	    <?php }else{?>
	      ...
	      <select class="form-control selectpicker" name="fpk_konfirm[]" id="fpk_konfirm" multiple data-live-search="true" data-live-search-style="begins" data-live-search-placeholder="Search" title="Pihak Approval" data-selected-text-format="count>3">
		<?php
		...
		<?php }}?> 
	      ...
	      <button type="submit" name="bkonfirm" class="btn btn-warning pull-right" <?php echo $done;?>><i class="fa fa-thumbs-up"></i> Konfirm</button>
	    ...
	    <?php }?>
	      ...
	      if($fpk_data_id['fpk_sts']=="Z"){
	      ?>
	      <button type="submit" name="bapproved" class="btn btn-success pull-right" <?php echo $done;?>><i class="fa fa-thumbs-up"></i> Approved</button>
	      <?php }else{?>
	      <button type="submit" name="bapproved" class="btn btn-success pull-right" <?php echo $konfirm;?>><i class="fa fa-thumbs-up"></i> Approved</button>
	      <?php }?>
