...
$tahun = substr($jdw_bulan,3,4 ) ;
$bln   = substr($jdw_bulan,0,2 ) ;
$blnthn = $bln."-".$tahun;

//echo 'tahun: '.$tahun.' bulan : '.$bln;
...
if(isset($page)&&($act=="UpdateSimpanLibur")){
//     echo 'Simpan Libur';
     $id        = $_REQUEST['id'];
     $jml_cuti  = $_REQUEST['jml_simpan_libur'];
	 $xdata_simpan = "";
	 $xdata_ambil  = "";
	 $jmlsl   	= 0 ;
	 if ($rec > 0){
	 ...
	 for($i=0;$i < $rec_max ; $i++){
	    $xdata_simpan 	.= $_REQUEST["tgl_simpan_libur"][$i];
	    $xdata_simpan 	.= "#";
	    $xdata_ambil 	.= $_REQUEST["data_ambil"][$i];
	    $xdata_ambil 	.= "#";
		if ($_REQUEST["tgl_simpan_libur"][$i] !="" and $_REQUEST["data_ambil"][$i]=="" ){
			$jmlsl++;
		}	
	 }   
        ...
        $q_upd    = "UPDATE cuti_master
	                 SET
					 jml_simpan_libur = '$jmlsl',
		  		     simpan_libur = '$xdata_simpan',
		  		     simpan_libur_ambil  = '$xdata_ambil'
				     WHERE kar_id='$id'   
			    ";
