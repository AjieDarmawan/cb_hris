...
<?php 
 ...
 // error_reporting(0);
   ...
   $title 		= "Pinjaman Paguyuban";
   $sub_title 	= "Pinjaman Paguyuban";
...
!-->
         	  <?php //}; ?>
				...
				<div class="box-body  table-responsive" >
				 <table id="tabel_paguyuban" width="98%"
				  ...
				  style="line-height:22px; " >
					...
					<thead>
						...
						<th  style="vertical-align:middle" >NIK / Pemohon</th>
						<th  style="vertical-align:middle">Jumlah Pinjaman</th>
						<th  style="vertical-align:middle; background-color:#00CCFF">ACC Bendahara</th>
						<th  style="vertical-align:middle; background-color:#00CC00">ACC Ketua</th>
						<th  style="vertical-align:middle" >Status Proses</th>
						<th  style="vertical-align:middle">Tenor</th>
						<th  style="vertical-align:middle">Angsuran</th>
						<th  style="vertical-align:middle">Angs Ke</th>
						<th  style="vertical-align:middle">Jumlah Bayar</th>
						<th  style="vertical-align:middle">Jumlah Pelunasan</th>
						<th  style="vertical-align:middle">Tanggal Pelunasan</th>
						<th  style="vertical-align:middle">Sisa</th>
						<th  style="vertical-align:middle">Tanggal Transfer</th>
						<th  style="vertical-align:middle">Update Potongan</th>
						<th  style="vertical-align:middle">Bank / NoRek</th>
						<th  style="vertical-align:middle">Cetak</th>
						</tr>
...
<form role="form" method="post" class="form-horizontal" id="save_update_konten"  enctype="multipart/form-data" > 
	...
	<input type="hidden" class="form-control" name="data_nik" id="data_nik" value="<?php echo  $data_nik;?>" >
...
*/
   ...
   ///////////////////////////////////////////////////////////////////////////////
	...
	function html_top_header(){
			...
			var fieldHTML = '';
			$.ajax({url: dir_url+"/pag_div_header_1.php",
...
///////////////////////////////////////////////////////////
var dataTable = $('#tabel_paguyuban').DataTable({
	//"responsive":true,
	"processing":true,
	"serverSide":true,
	"ajax":{
		"url": dir_url+"/pag_data_server.php",
		"type":"POST",
	
		"data":function(data){
				data.mode='list_data_paguyuban';
				data.id   = $('#data_id').val();
...
//	"order": [[ 1, 'asc' ],[ 2, 'asc' ]],	
	...
	"aoColumnDefs": [
		  {"bSortable": false,"aTargets": [ 0, 1, 2,7,8,9,10,11,12,13,14, -1 ]},
		 ...
		 // {"targets": 7, "render": $.fn.dataTable.render.number( ',', '.', 0 )},
		  {"targets": '_all',
		   ...
		   "createdCell": function (td, cellData, rowData, row, col) {
				///////set-panding/////////
				$(td).css('padding', '5px 5px 5px 0px');
		   }},
...
function html_update_daftar(act,id){
       // alert('id:'+id);
		...
		var aksi   		= "save_add_data";
		var aksi_proses = "edit_data";
		var sub_title 	= "Form Edit # " ;
	    ...
	    var xdata  = "";
		var fieldHTML = '';
		$.ajax({url: dir_url+"/pag_modal_daftar.php",
			async: false, // makes request synchronous //
			data:{ id			: id,
			       mode			: mode,
				   act          : act, 
     ...
     doDeleteDetail = function(data) {
		 if (confirm('Delete Record ?...'+data)==false){
		  ...
		  var mode   = "delete_nota_detail";
			  ...
			  var xdata  = "";
			  $.ajax({
					url: dir_url+'/pag_data_action.php',
					type: 'POST',
					dataType: 'json',
					async: false,
...
//////////////////////////////////////////////////////////////////////////////
	...
	function html_pelunasan(id){
			...
			var mode   		= "list_pinjaman";
			var aksi   		= "save_editdata_pelunasan";
			var aksi_proses = "edit_data_pelunasan";
			var sub_title 	= "Data Paguyuban" ;
			var user_nama   = "<?php echo $user_nama;?>" ;
			///////add-data/////////////////////////
			if (id==0){
				var aksi	    = "save_data_pelunasan" ;
				var aksi_proses = "add_data" ; 
				var sub_title 	= "Data Paguyuban " ;
			}
			var id_klp_order = $('#id_klp_order').val();
			////////////////////////////////////////		
			var xdata  = "";
			var fieldHTML = '';
		//	alert('xxxxx');
		//	alert(id+' '+no);
			...
			/////////////////////////////////////
			$.ajax({url: dir_url+"/form_pelunasan.php",
				async: false, // makes request synchronous //
				data:{ id			: id,
					   no			: no,
					   mode			: mode,
     ...
     doPelunasan = function(data) {
			$('#loading_data').modal({show: true,backdrop: 'static',keyboard: false});	
			var nama =  "";
			var mode = "edit";
			var fieldHTML  ='';
			$('#DIV_MODAL').html(fieldHTML);////////reset//////////////
			//////////////////////////////////////////////////////////	
			fieldHTML = html_pelunasan(data);
			$('#DIV_MODAL').html(fieldHTML);  /////////////add-html//////////
			$('.dp').datepicker({
				 todayHighlight: true,
				 format: "yyyy-mm-dd",
  ...
  /////////////////////////////////////////////////////////////////////
    doUpdateData = function(id , xcetak ) {
		...
		$('#modal-update-user').modal({
		  ...
		  if (document.getElementById("bmulai_bayar")){
			  document.getElementById("bmulai_bayar").disabled = true ;
			  document.getElementById("bmulai_bayar").style.display = "none" ;
		  }			  		 
    /////////////////////hanya-cetak///////////////////////////
		//CetakPrinter(cetak);
		//CekApprove(cetak) ; 
		
		if (xcetak == "hanya-cetak"    ||  xcetak == "approve-ketua" ||  xcetak == "approve-bendahara"  ||  
			xcetak == "bapprove_batal"  ){
			 // alert(xcetak);
	 ...
	 function CekApprove(cetak) {
			...
			if (cetak == "approve-ketua" || cetak == "approve-bendahara" ){
				  CekTOPRINT(data="xxx",cetak=true ) ;	
				  if (document.getElementById("checkbox_1")){
					  document.getElementById("checkbox_1").checked = true ;
					  document.getElementById("checkbox_1").disabled = true ;
...
//	doRefreshForm(data,labe);
   ...
   function CekTOPRINT(data,cetak=true) {
		...
		if (cek == undefined ){
		 ...
		 CekValueForm(data,label="pg_pinjaman") ;
		 CekValueForm(data,label="pg_lama") ;
		 CekValueForm(data,label="pg_ket") ; 
		 CekValueForm(data,label="pg_norek") ;
...
/////////////////////////////////////////////////////////////////	
	$('#save_update_konten_xxx').submit(function(e) {
	    ///////////cek-delete /////////////
		  ...
		  if (result.value) {
				$.ajax({
					...
					success: function(data) {
						...
						if(json.status == '1') {
							//////////jika closing///////////////
							//if (aksi_submit == "bclose" || aksi_submit == '' || aksi_submit == null){
								$('#modal-update-user').modal('hide');
								$('#save_update_konten').trigger("reset");
							//}
