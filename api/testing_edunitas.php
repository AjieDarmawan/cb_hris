...
$sSQL = '
	SELECT COUNT(a.id_testing) as tot
	FROM aktifitas_testing a
	WHERE 1=1
		' . $stw_whys . '
';
...
/*--- LIST DATANYA -------------------------------------------------------------------------------------------------------------------------------*/
$sSQL = '
	SELECT a.*,c.nama_karyawan as nickname, b.nama_testing, d.div_nm as label_divisi
	FROM aktifitas_testing a
	LEFT JOIN testing b ON a.id_aktifitas_testing = b.id_testing
	LEFT JOIN users c ON a.nik = c.nik
	LEFT JOIN divisi d ON c.divisi_id = d.div_id
	WHERE 1=1
		' . $stw_whys . '
	GROUP BY a.id_testing
	' . $sortby . '
	LIMIT ' . $no_awal . ', ' . $limit . '
';
...
if ($res_data->num_rows > 0) {
    ...
    while($row = $res_data->fetch_assoc()) {
		...
		unset($row['createdAt']);
		unset($row['updatedAt']);
		unset($row['id_testing']);
		unset($row['id_aktifitas_testing']);
		...
		$rowdata['aktifitas'] = $row['nama_testing'];
		$rowdata['keterangan'] = $row['nama_aktifitas'];
