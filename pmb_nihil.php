...
for($i=1; $i<=$ptsCount; $i++){
	...
	$syntac_union .= "(SELECT $field FROM `gg_reguler`.$regArr[$i] AS a,`gg_reguler`.datapt1 AS b,`gg_reguler`.datajrs AS c WHERE a.kodept = b.kodept3 AND a.kodept = c.kodept AND a.kodejrs=c.kodejrs AND a.kpi='$username' AND a.tgl_daftar BETWEEN '$r_awal_ori' AND '$r_sekarang_ori')".$union;
...
while($data = mysql_fetch_array($kpi_grab_bio)){
	...
	if($herregis > 0){
		...
		if(($data['potong_spb'] == '' || $data['potong_spb'] == NULL || $data['potong_spb'] == 0) &&
		   ...
		   ($data['smb_informasi'] !== 'Teman' && $data['smb_informasi'] !== 'Rekomendasi' && $data['smb_informasi'] !== 'Agency' && $data['smb_informasi'] !== 'GSF')){
